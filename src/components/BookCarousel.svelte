...
<div class="flex flex-row items-center">
	<button
		class="-mr-5 z-10 rounded-full text-white w-10 h-10 bg-black/50"
		on:click={() => {
			scroll(false);
		}}>{'<'}</button
	>
	...
	<button class="-ml-5 rounded-full text-white w-10 h-10 bg-black/50" on:click={() => scroll()}
		>{'>'}</button
	>
</div>
<h1>{`${currentIndex + 1}/${books.length}`}</h1>
